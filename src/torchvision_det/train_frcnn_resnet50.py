...
OUT_DIR = Path("runs/detect/frcnn_resnet50"); OUT_DIR.mkdir(parents=True, exist_ok=True)
...
def build_model(num_classes=2):
    ...
    # swap predictor for 2 classes (background + HAB)
...
def eval_loss(model, loader, dev):
    model.train()  # detection losses only in train mode
    total, n = 0.0, 0
...
def train(epochs_head=10, epochs_rpn=40, bs=4, img_size=640, lr=5e-4):
    ...
    for ep in range(1, epochs_head+1):
        ...
        print(f"[FRCNN-R50:head] ep {ep:02d}  val_loss={v:.4f}")
        if v < best:
            best = v
            torch.save(m.state_dict(), OUT_DIR / "best_head.pt")

    # -------- Phase B: fine-tune RPN + ROI head (backbone still frozen) --------
    unfreeze_rpn_and_head(m)  # already true, but be explicit
    params = [p for p in m.parameters() if p.requires_grad]
    ...
    for ep in range(1, epochs_rpn+1):
        ...
        v = eval_loss(m, val_dl, dev); sched.step()
        print(f"[FRCNN-R50:rpn+head] ep {ep:02d}  val_loss={v:.4f}")
        if v < best:
            best = v
