...
Key features:
  ...
  • Threshold scope:
     ...
     (no labels used) — fixes the mismatch when distributions shift.
  • Time-based CV: --cv_time_folds K  (train on earlier months, test on a month block)
...
Outputs:
  outdir/
    ...
    - summary_cv.csv (for cv_time_folds)
"""
...
def _clean_columns(df: pd.DataFrame, src: str = "") -> pd.DataFrame:
    ...
    if dup_mask.any():
        ...
        df = df.loc[:, ~dup_mask]; cols = [c for i, c in enumerate(cols) if not dup_mask[i]]
    df.columns = cols
...
def _coerce_id(df: pd.DataFrame, id_col: str, src: str) -> pd.DataFrame:
    ...
    if id_col not in df.columns:
        if "chip_id" in df.columns: df = df.rename(columns={"chip_id": id_col})
        elif "tile" in df.columns:   df = df.rename(columns={"tile": id_col})
        elif "image" in df.columns:  df[id_col] = df["image"].apply(lambda s: Path(str(s)).name)
        else: raise SystemExit(f"{src} has no id column '{id_col}'. Columns: {list(df.columns)}")
    return df
...
def _agg(df: pd.DataFrame, by_cols, score_name: str, how: str) -> pd.DataFrame:
    if how == "max":    return df.groupby(by_cols, as_index=False)[score_name].max()
    if how == "mean":   return df.groupby(by_cols, as_index=False)[score_name].mean()
    if how == "median": return df.groupby(by_cols, as_index=False)[score_name].median()
...
def _merge_on_scene(base, det_df_scene_like, group_col, score_name):
    ...
    base2 = base.copy(); base2["__scene_key__"] = base[group_col].astype(str).apply(lambda s: _canonical_scene_key(s))
    merged = base2.merge(det_tmp[["__scene_key__", score_name]], on="__scene_key__", how="left").drop(columns=["__scene_key__"], errors="ignore")
    if merged[score_name].isna().mean() < 1.0:
        cov = 100.0 * merged[score_name].notna().mean()
        print(f"[info] Detector '{score_name}' merged on scene. Coverage={cov:.1f}%")
        return merged, True
    det_tmp2 = det_df_scene_like.iloc[:, :2].copy(); det_tmp2.columns = ["__file__", score_name]
    ...
    merged2 = base2.merge(det_tmp2[["__scene_key__", score_name]], on="__scene_key__", how="left").drop(columns=["__scene_key__"], errors="ignore")
    if merged2[score_name].isna().mean() < 1.0:
        cov = 100.0 * merged2[score_name].notna().mean()
...
def _merge_on_month_region(base, det_df, score_name, agg: str, month_backfill: int = 0):
    ...
    if {"month_key",score_name}.issubset(det_ok.columns):
        ...
        if month_backfill and len(det_m):
            ...
            base_remap = base.copy(); base_remap["__det_month__"] = base_remap["month_key"].map(nn_map)
            det_m2 = det_m.rename(columns={"month_key":"__det_month__"})
            m3 = base_remap.merge(det_m2, on="__det_month__", how="left").drop(columns="__det_month__", errors="ignore")
            if m3[score_name].isna().mean() < 1.0:
                cov = 100.0 * m3[score_name].notna().mean()
                print(f"[info] Detector '{score_name}' merged via nearest-month ±{month_backfill} ({agg}). Coverage={cov:.1f}%")
                return m3, True

    return base, False

def _merge_on_nearest_date(base, det_df, score_name, max_day_gap: int = 12, use_region: bool = True):
    if "date_key" not in base.columns or "date_key" not in det_df.columns: return base, False
    b = base.dropna(subset=["date_key"]).sort_values("date_key")
    d = det_df.dropna(subset=["date_key"]).sort_values("date_key")
    if b.empty or d.empty: return base, False
    ...
    if use_region and "region_key" in b.columns and "region_key" in d.columns:
        ...
        for rk, g in b.groupby("region_key"):
            ...
            if dd.empty:
                parts.append(g.assign(**{score_name: np.nan})); continue
            mg = pd.merge_asof(g, dd[["date_key",score_name]].sort_values("date_key"),
                               on="date_key", direction="nearest",
                               tolerance=pd.Timedelta(days=max_day_gap))
            parts.append(mg)
        merged = pd.concat(parts, axis=0).sort_index()
    else:
        merged = pd.merge_asof(b, d[["date_key",score_name]].sort_values("date_key"),
                               on="date_key", direction="nearest",
                               tolerance=pd.Timedelta(days=max_day_gap))
    out = base.copy(); out[score_name] = merged.reindex(base.index)[score_name]
...
# --------------- thresholding ---------------
def _pick_threshold_from_policy(y, p, prec, rec, thr,
                                policy="f1",
    ...
    if policy == "precision" and target_precision is not None:
        ...
        if len(cand): return float(thr[cand[0]]), "precision"

    if policy == "recall" and target_recall is not None:
        cand = np.where(rec[:-1] >= target_recall)[0]
        if len(cand): return float(thr[cand[-1]]), "recall"
...
def _eval_and_save(outdir: Path, tag: str, feats, y_true, p, thr_star, ids_df):
    ...
    preds = ids_df.copy()
    preds["p_fused"] = p; preds["yhat"] = yhat; preds["thr_star"] = thr_star
    ...
    (outdir / f"metrics_{tag}.json").write_text(json.dumps({
        "feats": feats, "auprc": float(auprc),
        "auroc": float(auroc) if not np.isnan(auroc) else None,
        "thr_star": float(thr_star), "base_rate": base_rate,
        "cm": cm, "n": int(len(y_true)), "pos": int(y_true.sum()),
        "unique_scores": uniq
    }, indent=2))
...
def main():
    ...
    ap.add_argument("--tabular_csv", required=True,
                    help="CSV with p_tab and labels (id_col, scene_id, hab_label, p_tab[, datetime])")
    ...
    # thresholding
    ap.add_argument("--threshold_policy",
                    choices=["f1","precision","recall","fpr","topfrac","expected_pos"], default="f1")
    ...
    ap.add_argument("--threshold_scope", choices=["train","test_unsupervised"], default="train",
    ...
    ap.add_argument("--normalize_detectors_only", action="store_true",
                    help="If set, normalize only detector columns; keep p_tab as-is when normalizing.")
    ...
    for spec in args.det:
        ...
        if id2name is not None:
            ...
            if ser.notna().mean() >= 0.90:
                before = len(df)
                df[args.id_col] = ser.map(lambda k: id2name.get(int(k)) if pd.notna(k) and int(k) in id2name else None)
        ...
        if score_col != name: df = df.rename(columns={score_col: name})
        df = _attach_month_region_keys_for_det(df, args.id_col)

        merged, ok = _merge_on_id(base, df[[args.id_col, name]], args.id_col, name)
        if not ok:
            det_scene_df = df[[args.id_col, name]].copy(); det_scene_df.columns = ["__file__", name]
            merged, ok = _merge_on_scene(base, det_scene_df, args.group_by, name)
        if not ok:
            merged, ok = _merge_on_month_region(base, df[[args.id_col,"month_key","region_key",name]],
                                                name, args.det_agg, month_backfill=args.month_backfill)
        if not ok:
            merged, ok = _merge_on_nearest_date(base, df[[args.id_col,"date_key","region_key",name]],
                                                name, max_day_gap=args.max_day_gap, use_region=True)
        if not ok:
            print(f"[warn] Detector '{name}' 0% coverage after all strategies.")
            if args.require_overlap: raise SystemExit(f"[fatal] '{name}' contributed 0 matches.")
        base = merged
        if name not in base.columns: base[name] = np.nan
    ...
    if args.intersection_only and det_names:
        ...
        kept = int(mask_all.sum()); total = int(len(base))
        if kept == 0: raise SystemExit("[intersection_only] No rows remain.")
    ...
    if args.drop_p_tab and "p_tab" in feats:
        ...
        print("[info] Dropping p_tab; using detectors only:", feats)
    ...
    if args.no_missing_flags:
        ...
        for col in feats:
            mcol = f"{col}_missing"
            base[mcol] = base[col].isna().astype(float)
            base[col] = base[col].astype(float).fillna(0.0)
            miss_cols.append(mcol)
        feats += miss_cols
    ...
    def _fit_predict(train_idx, test_idx, tag):
        ...
        if args.normalize_scores:
            if args.normalize_detectors_only:
                ...
                det_cols = [c for c in feat_cols if c not in protect and not c.endswith("_missing")]
                used = _normalize_inplace(df_train, df_test, det_cols, protect_cols=protect)
                print(f"[norm] normalized (detectors only): {used}")
            else:
                cols = [c for c in feat_cols if not c.endswith("_missing")]
                used = _normalize_inplace(df_train, df_test, cols)
                print(f"[norm] normalized (all score cols): {used}")
        ...
        base_clf = Pipeline([
            ...
            ("clf", LogisticRegression(max_iter=1000, class_weight="balanced", random_state=final_seed))
        ])
        ...
        thr_star, how = _pick_threshold_from_policy(
            y_train, p_tr, prec_tr, rec_tr, thr_tr,
        ...
        if args.threshold_scope == "test_unsupervised":
            if args.threshold_policy == "expected_pos" and args.expected_pos_rate is not None:
                ...
                print(f"[thr] Overriding thr* on TEST via expected_pos(rate={args.expected_pos_rate:.3f}) "
                ...
                thr_test = float(np.quantile(p_te, q)) if p_te.size else thr_star
                print(f"[thr] Overriding thr* on TEST via topfrac(frac={args.top_frac:.3f}) "
                      f"=> test-quantile={q:.3f} thr_test={thr_test:.3f}")
                thr_star = thr_test
            elif args.threshold_policy == "fpr" and args.target_fpr is not None:
                q = max(0.0, min(1.0, 1.0 - float(args.target_fpr)))
                thr_test = float(np.quantile(p_te, q)) if p_te.size else thr_star
                print(f"[thr] Overriding thr* on TEST via fpr≈quantile(target_fpr={args.target_fpr:.3f}) "
                      f"=> test-quantile={q:.3f} thr_test={thr_test:.3f}")
                thr_star = thr_test
        ...
        print(f"[score-q:{tag}] train pos q50={_q(pos_tr,0.5):.3f} q90={_q(pos_tr,0.9):.3f} | "
        ...
        joblib.dump({"model": clf, "features": feat_cols, "args": vars(args)}, outdir / f"fusion_model_{tag}.joblib")
        return auprc, auroc
    ...
    def _month_to_ordinal(s: str | None) -> int:
        if not isinstance(s, str) or "-" not in s: return -10**9
        try: y, m = s.split("-"); return int(y) * 12 + int(m)
    ...
    if args.cv_time_folds and args.cv_time_folds > 1:
        ...
        for fold_id, block in enumerate(month_blocks, 1):
            ...
            tr = np.where(train_mask)[0]; te = np.where(test_mask)[0]

            if len(tr) == 0:
                print(f"[cv-time] Skipping fold {fold_id} (no earlier months to train)."); continue
            if base.loc[tr, "hab_label"].sum() < args.min_pos_per_split or base.loc[te, "hab_label"].sum() < args.min_pos_per_split:
                print(f"[cv-time] Skipping fold {fold_id} (insufficient positives)."); continue

            print(f"[CV{fold_id}] (time) train={len(tr)} (pos={int(base.loc[tr,'hab_label'].sum())}) | "
                  f"test={len(te)} (pos={int(base.loc[te,'hab_label'].sum())}) | months_test={[int(b) for b in block]}")
        ...
        for fold_id, (tr, te) in enumerate(sgkf.split(X_all, y_all, groups), 1):
            if y_all[tr].sum() < args.min_pos_per_split or y_all[te].sum() < args.min_pos_per_split:
                print(f"[cv] Skipping fold {fold_id} (insufficient positives)."); continue
            print(f"[CV{fold_id}] train={len(tr)} (pos={int(y_all[tr].sum())}) | test={len(te)} (pos={int(y_all[te].sum())})")
            auprc, auroc = _fit_predict(tr, te, f"cv{fold_id}")
            summaries.append({"fold": fold_id, "auprc": auprc, "auroc": auroc,
        ...
        for _ in range(1, args.max_tries + 1):
            ...
            if y_all[tr].sum() >= args.min_pos_per_split and y_all[te].sum() >= args.min_pos_per_split:
                good = (tr, te); break
        if good is None: raise SystemExit("Could not find a split with positives in both partitions.")
        tr, te = good
        print(f"[Split] train={len(tr)} (pos={int(y_all[tr].sum())}) | test={len(te)} (pos={int(y_all[te].sum())})")
        auprc, auroc = _fit_predict(tr, te, "holdout")
        pd.DataFrame([{
            "model": "fusion(logreg)", "feats": "+".join(feats),
